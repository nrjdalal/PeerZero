...
- **Media player:** a playable file (video/audio, `isPlayable` in `file-tree.tsx`) shows a ▶ affordance - the file icon swaps to a play glyph on hover (no layout shift, so columns stay aligned), and Enter/Space on a focused file plays it. **Playback is adaptive** (`lib/playback/`): `detectCapabilities()` probes this machine's decode support (WebCodecs `VideoDecoder`/`AudioDecoder` + a native `<video>`), and `pickStrategy(name, caps)` routes each file to the cheapest path that works, `native` > `libmedia` > `handoff`:
  - **native** - browser-safe (mp4/webm + H.264/AAC): an inline **Vidstack** player (`components/torrents/player.tsx`), a full-screen `Dialog` filling the viewport (`h-svh w-svw`) on a black backdrop with no chrome or padding, the video letterboxing itself, and a corner `DialogClose` back to the app.
  - **libmedia** - mkv/HEVC/AC3/E-AC3/DTS: an in-browser **@libmedia** player (`components/torrents/libmedia-player.tsx`, same full-screen black shell) that demuxes the container and decodes via hardware WebCodecs when the codec is available (e.g. HEVC in a macOS WebView), else its own FFmpeg WASM (AC3/DTS have no WebCodecs path anywhere). `enableWorker:true` runs io/demux/decode/render on worker threads so buffering ahead never freezes the UI; no COOP/COEP needed (no SharedArrayBuffer). It **self-hosts** the ESM chunks + codec WASM from `/public/libmedia` (vendored by `.github/scripts/vendor-libmedia.ts`), so it runs fully offline with no CDN.
  - **handoff** - when only a software WASM decode would be left on desktop (no hardware decoder for the file's video), or when libmedia errors: hand the stream to a native player via `@tauri-apps/plugin-opener` `openUrl(url, "VLC")`. In a plain browser (no handoff) libmedia is the best-effort path, and a toast explains when nothing can play.
