...
- **Media player:** a playable file (video/audio, `isPlayable` in `file-tree.tsx`) shows a ▶ affordance - the file icon swaps to a play glyph on hover (no layout shift, so columns stay aligned), and Enter/Space on a focused file plays it. Playback is codec-aware (`lib/play-file.ts`): **browser-safe** formats (mp4/webm + H.264/AAC) open an inline **Vidstack** player (`components/torrents/player.tsx`, a full-screen `Dialog`: the content fills the viewport (`h-svh w-svw`) on a black backdrop with no chrome or padding, the video letterboxes itself, and a single corner `DialogClose` returns to the app); everything else (mkv/HEVC/AC3/DTS) hands the stream to a **native player** via `@tauri-apps/plugin-opener` `openUrl(url, "VLC")` on desktop, or toasts an "open in the desktop app / VLC" message in a plain browser - because browsers decode the video but not AC3/DTS audio (or the mkv container at all). The stream is a Range endpoint (`GET /api/torrents/:infoHash/stream/:fileIdx`, absolute `config.api.url`, no credentials, exempt from the rate limiter) proxying `file.createReadStream` from the engine. Decoding those non-browser-safe codecs **in-browser** was evaluated and deliberately deferred: it needs ffmpeg.wasm (~30 MB payload, marginal 1080p HEVC performance) or WebCodecs where HEVC/AC3 aren't universally available, for something the native handoff already does perfectly. So the handoff is the answer, not an embedded decoder.
