...
Start the dev servers (`dev` skill). Under the default portless dev the base URLs are named and branch-prefixed, so resolve them once: `WEB=$(bunx portless get peerzero)` and `API=$(bunx portless get api.peerzero)` (or `PORTLESS=0 bun run dev` for fixed `http://localhost:9410` / `http://localhost:9336`). Done when `$WEB/` returns 200 and `$API/api/health` responds ok.
