...
| Change how finished torrents are organized into a media library | `api/torrent-engine/src/media-library.mjs` (release parser + Jellyfin `Movies/`, `Shows/` layout with a quality "version" suffix + collision-safe hardlink primitive), wired in `index.mjs` via `linkToLibrary` on the torrent `done` event + the persisted `mediaLibrary` settings block. Default folder is auto (`<downloadDir>/Media`, so same-drive is guaranteed); an explicit folder overrides it. Only video files are linked; the canonical download is never moved/renamed; a second release of the same title becomes a ` (2)` version. Configured in `web/next/src/components/torrents/settings-dialog.tsx` (toggle + folder). `libraryLinked` flows `snapshot()` -> `TorrentSnapshot` | - |
...
| Change how a torrent's clean display name is generated | `web/next/src/lib/torrent-name/` (deterministic parser + on-device Prompt API behind a small `TorrentNameGenerator`) + `use-auto-display-name.ts` (fires once per genuinely-new torrent). The optional `displayName` flows engine `snapshot()` -> `TorrentSnapshot` -> UI (`displayName ?? name`); reveal/delete always use the canonical `name` | - |
