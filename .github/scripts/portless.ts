...
// Toggle the adjacent `api.` label (it sits just before the two base labels) to get the sibling
// api host. Assumes the `<name>` / `api.<name>` naming from the package.json `portless.name`s
...
export function deriveUrls(portlessUrl: string): { web: string; api: string } {
  ...
  // Insert the `api` sibling label just before the base name so the worktree branch prefix
  // carries over to the api host.
  const apiLabels = isApi ? labels : webLabels.toSpliced(webLabels.length - 2, 0, "api")
